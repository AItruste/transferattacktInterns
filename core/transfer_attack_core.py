...
# BSR: Block Shuffle and Rotation (CVPR 2024)
...
def _bsr_shuffle_rotate(x, num_block: int = 2):
    ...
    w_strips = tf.split(x, _bsr_get_lengths(W, num_block), axis=2)
    result_w = []
    for wi in np.random.permutation(num_block).tolist():
        h_strips = tf.split(w_strips[wi], _bsr_get_lengths(H, num_block), axis=1)
        result_h = []
